"""Arkansas filing runner for the Online_Compliance_Bot project."""
...
AR_HOLDER_INFO_URL = "https://claimitar.gov/app/holder-info"


class ArkansasAutomationError(RuntimeError):
    """Raised when AR automation cannot reliably continue."""
...
async def run(
    ...
    *,
    ...
    await page.goto(AR_HOLDER_INFO_URL, wait_until="domcontentloaded")
    await page.wait_for_timeout(wait_after_navigation_ms)
    await page.get_by_label("Holder Name").first.wait_for(timeout=20_000)

    errors: list[str] = []
    await _fill_ar_holder_info_page(page, record, errors)

    if errors:
        raise ArkansasAutomationError("AR holder-info form completed with errors:\n- " + "\n- ".join(errors))

    print("AR debug -> clicking Next after AR holder info completed")
    await click_next(page, "after AR holder info")
    await _upload_naupa_file(page, naupa_path)


async def run_arkansas(
    page: Page,
    holder_row: Dict[str, Any],
    payment_row: Dict[str, Any],
    naupa_file_path: str | Path,
...
async def _fill_ar_holder_info_page(page: Page, record: Dict[str, Any], errors: list[str]) -> None:
    for field in _TEXT_FIELDS:
        ...
        await _guarded(errors, f"text '{field.label}'", lambda f=field, v=value: fill_text_field(page, f.label, v, "AR"))

    report_type = _as_string(record.get("report_type"))
    if not report_type:
        errors.append("report_type is required for 'Report Type'.")
    else:
        print(f"AR debug -> Report Type mapped from report_type='{report_type}'")
        await _guarded(errors, "dropdown 'Report Type'", lambda: select_dropdown_field(page, "Report Type", report_type, "AR"))
    ...
    if negative is None:
        negative = False
    print(f"AR debug -> This is a Negative Report mapped from negative_report='{('Yes' if negative else 'No')}'")
    await _guarded(errors, "radio 'This is a Negative Report'", lambda: set_radio_field(page, "This is a Negative Report", negative, "AR"))
    ...
    if not amount:
        ...
        print(f"AR debug -> Total Dollar Amount Remitted mapped from amount_to_remit='{amount}'")
        await _guarded(errors, "text 'Total Dollar Amount Remitted'", lambda: fill_text_field(page, "Total Dollar Amount Remitted", amount, "AR"))


def _print_text_debug(field: _TextFieldSpec, value: str) -> None:
    if field.label == "Email Address Confirmation":
        print(f"AR debug -> Email Confirmation reused from email='{value}'")
    else:
        print(f"AR debug -> {field.label} mapped from {field.key}='{value}'")


async def _set_report_year_if_enabled(page: Page, report_year: str) -> None:
    try:
        row, _ = await locate_strict_row_for_label(page, "Report Year", "dropdown", "AR")
        ...
        if disabled_or_readonly:
            print("AR debug -> skipping Report Year (disabled field)")
            return

        if not report_year:
            return

        print(f"AR debug -> Report Year mapped from report_year='{report_year}'")
        try:
...
async def _wait_for_upload_page(page: Page) -> None:
    try:
        ...
        print("AR debug -> reached holder-upload page")
        return
    ...
    raise ArkansasAutomationError("AR did not reach holder-upload after holder info Next.")
...
async def _upload_naupa_file(page: Page, file_path: Path) -> None:
    ...
    if not file_path.exists():
        raise ArkansasAutomationError(f"AR NAUPA file does not exist: {file_path}")

    print(f"AR debug -> uploading AR NAUPA file: {file_path}")
    ...
    if not uploaded:
        raise ArkansasAutomationError("Could not find AR upload file input.")

    await page.wait_for_timeout(1500)
    print("AR debug -> NAUPA uploaded; clicking upload-page Next")
    await click_next(page, "after AR upload")
    await _wait_for_preview_or_signature(page)
    print("AR debug -> reached holder-preview; waiting for manual signature")
    print("AR finished - waiting for manual signature")
...
async def _wait_for_preview_or_signature(page: Page) -> None:
    ...
    raise ArkansasAutomationError("AR upload did not reach holder-preview or signature prompt.")
...
async def click_next(page: Page, context: str) -> None:
    for candidate in (
        ...
        for i in range(count):
            ...
            return
    raise ArkansasAutomationError(f"Could not find a clickable Next control {context}.")
